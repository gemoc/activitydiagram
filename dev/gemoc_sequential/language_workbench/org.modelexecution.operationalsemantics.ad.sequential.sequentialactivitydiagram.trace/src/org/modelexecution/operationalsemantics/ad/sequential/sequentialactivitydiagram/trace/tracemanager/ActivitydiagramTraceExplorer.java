...
public class ActivitydiagramTraceExplorer implements ITraceExplorer {
	...
	private activitydiagramTrace.Steps.SpecificStep findNextStep(
		...
		while (result == null && i < depth) {
			...
			if (currentStep instanceof SequentialStep<?>) {
				currentSubSteps.addAll(((SequentialStep<activitydiagramTrace.Steps.SpecificStep>) currentStep)
						.getSubSteps());
			}
	...
	private void computeExplorerState(List<activitydiagramTrace.Steps.SpecificStep> stepPath) {
		...
		final List<activitydiagramTrace.Steps.SpecificStep> stepPathUnmodifiable = Collections.unmodifiableList(stepPath);
			...
			for (activitydiagramTrace.States.ActivityNode_heldTokens_Value value : stateToGo
				...
				if (value.getParent() instanceof activitydiagramTrace.States.activitydiagram.TracedActivityFinalNode) {
					activitydiagramTrace.States.activitydiagram.TracedActivityFinalNode parent_cast = (activitydiagramTrace.States.activitydiagram.TracedActivityFinalNode) value
							.getParent();
					activitydiagram.ActivityNode originalObject = (activitydiagram.ActivityNode) parent_cast
							.getOriginalObject();
					originalObject.getHeldTokens().clear();
					originalObject.getHeldTokens().addAll(
			...
			for (activitydiagramTrace.States.ActivityEdge_offers_Value value : stateToGo
				...
				if (value.getParent() instanceof activitydiagramTrace.States.activitydiagram.TracedControlFlow) {
					activitydiagramTrace.States.activitydiagram.TracedControlFlow parent_cast = (activitydiagramTrace.States.activitydiagram.TracedControlFlow) value
							.getParent();
					activitydiagram.ActivityEdge originalObject = (activitydiagram.ActivityEdge) parent_cast
							.getOriginalObject();
					originalObject.getOffers().clear();
					originalObject.getOffers().addAll(
		...
		if (traceIndex > -1 && traceIndex < valueTraces.size()) {
			return canBackValueCache
					.computeIfAbsent(
							traceIndex,
							i -> {
								final List<? extends activitydiagramTrace.States.Value> valueTrace = valueTraces
										.get(traceIndex);
	...
	@Override
	public List<activitydiagramTrace.Steps.SpecificStep> getStepsForStates(int startingState, int endingState) {
	...
	private boolean isStateBreakable(activitydiagramTrace.States.State state) {
		...
		if (b) {
			...
			return !(s instanceof activitydiagramTrace.Steps.Activitydiagram_ActivityFinalNode_Execute_ImplicitStep
					|| s instanceof activitydiagramTrace.Steps.Activitydiagram_ActivityNode_AddTokens1_ImplicitStep
					...
					|| s instanceof activitydiagramTrace.Steps.Activitydiagram_Activity_WriteTrace_ImplicitStep
					|| s instanceof activitydiagramTrace.Steps.Activitydiagram_BooleanVariable_Execute_ImplicitStep
					|| s instanceof activitydiagramTrace.Steps.Activitydiagram_DecisionNode_Execute_ImplicitStep
					|| s instanceof activitydiagramTrace.Steps.Activitydiagram_ForkNode_Execute_ImplicitStep
					|| s instanceof activitydiagramTrace.Steps.Activitydiagram_InitialNode_Execute_ImplicitStep
					|| s instanceof activitydiagramTrace.Steps.Activitydiagram_JoinNode_Execute_ImplicitStep
					|| s instanceof activitydiagramTrace.Steps.Activitydiagram_MergeNode_Execute_ImplicitStep
					|| s instanceof activitydiagramTrace.Steps.Activitydiagram_NamedElement_Execute_ImplicitStep || s instanceof activitydiagramTrace.Steps.Activitydiagram_OpaqueAction_Execute_ImplicitStep);
		}
		return true;
	}
	...
	public StateWrapper getStateWrapper(int stateIndex) {
		if (stateIndex > -1 && stateIndex < statesTrace.size()) {
			...
			return new StateWrapper(state, stateIndex);
		}
		return null;
	}
	...
	public List<StateWrapper> getStateWrappers(int start, int end) {
		...
		for (int i = startStateIndex; i < endStateIndex + 1; i++) {
			...
			result.add(new StateWrapper(state, i));
		}
