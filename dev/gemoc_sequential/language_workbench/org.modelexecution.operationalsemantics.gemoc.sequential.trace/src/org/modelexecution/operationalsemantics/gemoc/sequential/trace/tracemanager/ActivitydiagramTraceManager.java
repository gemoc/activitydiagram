...
public class ActivitydiagramTraceManager implements fr.inria.diverse.trace.gemoc.api.IGemocTraceManager {
	...
	private static final List<String> bigSteps = Arrays.asList("Activitydiagram_Activity_WriteTrace",
			"Activitydiagram_Activity_Execute", "Activitydiagram_ActivityNode_TakeOfferdTokens1",
			"Activitydiagram_ActivityNode_AddTokens1", "Activitydiagram_ActivityNode_Execute");
		...
		for (Resource r : allResources)
			for (TreeIterator<EObject> i = r.getAllContents(); i.hasNext();) {
				...
				if (o instanceof activitydiagram.ActivityFinalNode) {
					...
					List<activitydiagramTrace.States.ActivityNode_heldTokens_Value> localTrace1 = tracedObject
							.getHeldTokensSequence();
					activitydiagramTrace.States.ActivityNode_heldTokens_Value previousValue1 = null;
					...
					if (previousValue1 != null) {
						...
						if (previousValue1.getHeldTokens().size() == o_cast.getHeldTokens().size()) {

							java.util.Iterator<activitydiagram.Token> it = o_cast.getHeldTokens().iterator();
							for (activitydiagramTrace.States.activitydiagram.TracedToken aPreviousValue : previousValue1
									.getHeldTokens()) {
					...
					if (noChange1) {
						newState.getActivityNode_heldTokens_Values().add(previousValue1);
					...
					// Then we compare the value of the field with the last stored value
					// If same value, we create no local state and we refer to the previous
					List<activitydiagramTrace.States.ActivityNode_incoming_Value> localTrace2 = tracedObject
							.getIncomingSequence();
					activitydiagramTrace.States.ActivityNode_incoming_Value previousValue2 = null;
					...
					if (previousValue2 != null) {
						...
						if (previousValue2.getIncoming().size() == o_cast.getIncoming().size()) {

							java.util.Iterator<activitydiagram.ActivityEdge> it = o_cast.getIncoming().iterator();
							for (activitydiagramTrace.States.activitydiagram.TracedActivityEdge aPreviousValue : previousValue2
									.getIncoming()) {
					...
					if (noChange2) {
						newState.getActivityNode_incoming_Values().add(previousValue2);
					...
					// Then we compare the value of the field with the last stored value
					// If same value, we create no local state and we refer to the previous
					List<activitydiagramTrace.States.ActivityNode_running_Value> localTrace3 = tracedObject
							.getRunningSequence();
					activitydiagramTrace.States.ActivityNode_running_Value previousValue3 = null;
					if (!localTrace3.isEmpty())
						previousValue3 = localTrace3.get(localTrace3.size() - 1);

					boolean content1 = o_cast.isRunning();

					boolean noChange3 = previousValue3 != null && previousValue3.isRunning() == content1;

					if (noChange3) {
						newState.getActivityNode_running_Values().add(previousValue3);
					...
					// Then we compare the value of the field with the last stored value
					// If same value, we create no local state and we refer to the previous
					List<activitydiagramTrace.States.ActivityNode_activity_Value> localTrace4 = tracedObject
							.getActivitySequence();
					activitydiagramTrace.States.ActivityNode_activity_Value previousValue4 = null;
					if (!localTrace4.isEmpty())
						previousValue4 = localTrace4.get(localTrace4.size() - 1);

					storeAsTracedObject(o_cast.getActivity());

					activitydiagramTrace.States.activitydiagram.TracedActivity content2 = null;
					if (o_cast.getActivity() != null)
						content2 = ((activitydiagramTrace.States.activitydiagram.TracedActivity) exeToTraced.get(o_cast
								.getActivity()));

					boolean noChange4 = previousValue4 != null && previousValue4.getActivity() == content2;

					if (noChange4) {
						newState.getActivityNode_activity_Values().add(previousValue4);
					...
					// Then we compare the value of the field with the last stored value
					// If same value, we create no local state and we refer to the previous
					List<activitydiagramTrace.States.NamedElement_name_Value> localTrace5 = tracedObject
							.getNameSequence();
					activitydiagramTrace.States.NamedElement_name_Value previousValue5 = null;
					if (!localTrace5.isEmpty())
						previousValue5 = localTrace5.get(localTrace5.size() - 1);

					java.lang.String content3 = o_cast.getName();

					boolean noChange5 = previousValue5 != null && previousValue5.getName() == content3;

					if (noChange5) {
						newState.getNamedElement_name_Values().add(previousValue5);
					...
					// Then we compare the value of the field with the last stored value
					// If same value, we create no local state and we refer to the previous
					List<activitydiagramTrace.States.ActivityNode_outgoing_Value> localTrace6 = tracedObject
							.getOutgoingSequence();
					activitydiagramTrace.States.ActivityNode_outgoing_Value previousValue6 = null;
					...
					if (previousValue6 != null) {
						...
						if (previousValue6.getOutgoing().size() == o_cast.getOutgoing().size()) {

							java.util.Iterator<activitydiagram.ActivityEdge> it = o_cast.getOutgoing().iterator();
							for (activitydiagramTrace.States.activitydiagram.TracedActivityEdge aPreviousValue : previousValue6
									.getOutgoing()) {
								activitydiagram.ActivityEdge aCurrentValue = it.next();
								if (aPreviousValue != exeToTraced.get(aCurrentValue)) {
									noChange6 = false;
					...
					if (noChange6) {
						newState.getActivityNode_outgoing_Values().add(previousValue6);
				...
				if (o instanceof activitydiagram.IntegerCalculationExpression) {
					...
					// Then we compare the value of the field with the last stored value
					// If same value, we create no local state and we refer to the previous
					List<activitydiagramTrace.States.IntegerCalculationExpression_operator_Value> localTrace7 = tracedObject
							.getOperatorSequence();
					activitydiagramTrace.States.IntegerCalculationExpression_operator_Value previousValue7 = null;
					if (!localTrace7.isEmpty())
						previousValue7 = localTrace7.get(localTrace7.size() - 1);

					activitydiagram.IntegerCalculationOperator content4 = o_cast.getOperator();

					boolean noChange7 = previousValue7 != null && previousValue7.getOperator() == content4;

					if (noChange7) {
						newState.getIntegerCalculationExpression_operator_Values().add(previousValue7);
					...
					// Then we compare the value of the field with the last stored value
					// If same value, we create no local state and we refer to the previous
					List<activitydiagramTrace.States.IntegerExpression_operand2_Value> localTrace8 = tracedObject
							.getOperand2Sequence();
					activitydiagramTrace.States.IntegerExpression_operand2_Value previousValue8 = null;
					if (!localTrace8.isEmpty())
						previousValue8 = localTrace8.get(localTrace8.size() - 1);

					storeAsTracedObject(o_cast.getOperand2());

					activitydiagramTrace.States.activitydiagram.TracedIntegerVariable content5 = null;
					if (o_cast.getOperand2() != null)
						content5 = ((activitydiagramTrace.States.activitydiagram.TracedIntegerVariable) exeToTraced
								.get(o_cast.getOperand2()));

					boolean noChange8 = previousValue8 != null && previousValue8.getOperand2() == content5;

					if (noChange8) {
						newState.getIntegerExpression_operand2_Values().add(previousValue8);
					...
					// Then we compare the value of the field with the last stored value
					// If same value, we create no local state and we refer to the previous
					List<activitydiagramTrace.States.IntegerCalculationExpression_assignee_Value> localTrace9 = tracedObject
							.getAssigneeSequence();
					activitydiagramTrace.States.IntegerCalculationExpression_assignee_Value previousValue9 = null;
					if (!localTrace9.isEmpty())
						previousValue9 = localTrace9.get(localTrace9.size() - 1);

					storeAsTracedObject(o_cast.getAssignee());

					activitydiagramTrace.States.activitydiagram.TracedIntegerVariable content6 = null;
					if (o_cast.getAssignee() != null)
						content6 = ((activitydiagramTrace.States.activitydiagram.TracedIntegerVariable) exeToTraced
								.get(o_cast.getAssignee()));

					boolean noChange9 = previousValue9 != null && previousValue9.getAssignee() == content6;

					if (noChange9) {
						newState.getIntegerCalculationExpression_assignee_Values().add(previousValue9);
					...
					// Then we compare the value of the field with the last stored value
					// If same value, we create no local state and we refer to the previous
					List<activitydiagramTrace.States.IntegerExpression_operand1_Value> localTrace10 = tracedObject
							.getOperand1Sequence();
					activitydiagramTrace.States.IntegerExpression_operand1_Value previousValue10 = null;
					if (!localTrace10.isEmpty())
						previousValue10 = localTrace10.get(localTrace10.size() - 1);

					storeAsTracedObject(o_cast.getOperand1());

					activitydiagramTrace.States.activitydiagram.TracedIntegerVariable content7 = null;
					if (o_cast.getOperand1() != null)
						content7 = ((activitydiagramTrace.States.activitydiagram.TracedIntegerVariable) exeToTraced
								.get(o_cast.getOperand1()));

					boolean noChange10 = previousValue10 != null && previousValue10.getOperand1() == content7;

					if (noChange10) {
						newState.getIntegerExpression_operand1_Values().add(previousValue10);
				...
				if (o instanceof activitydiagram.Offer) {
					...
					// Then we compare the value of the field with the last stored value
					// If same value, we create no local state and we refer to the previous
					List<activitydiagramTrace.States.Offer_offeredTokens_Value> localTrace11 = tracedObject
							.getOfferedTokensSequence();
					activitydiagramTrace.States.Offer_offeredTokens_Value previousValue11 = null;
					...
					if (previousValue11 != null) {
						...
						if (previousValue11.getOfferedTokens().size() == o_cast.getOfferedTokens().size()) {
					...
					if (noChange11) {
						newState.getOffer_offeredTokens_Values().add(previousValue11);
				...
				if (o instanceof activitydiagram.IntegerValue) {
					...
					// Then we compare the value of the field with the last stored value
					// If same value, we create no local state and we refer to the previous
					List<activitydiagramTrace.States.IntegerValue_value_Value> localTrace12 = tracedObject
							.getValueSequence();
					activitydiagramTrace.States.IntegerValue_value_Value previousValue12 = null;
					if (!localTrace12.isEmpty())
						previousValue12 = localTrace12.get(localTrace12.size() - 1);

					int content8 = o_cast.getValue();

					boolean noChange12 = previousValue12 != null && previousValue12.getValue() == content8;

					if (noChange12) {
						newState.getIntegerValue_value_Values().add(previousValue12);
				...
				if (o instanceof activitydiagram.Activity) {
					...
					// Then we compare the value of the field with the last stored value
					// If same value, we create no local state and we refer to the previous
					List<activitydiagramTrace.States.Activity_edges_Value> localTrace13 = tracedObject
							.getEdgesSequence();
					activitydiagramTrace.States.Activity_edges_Value previousValue13 = null;
					...
					if (previousValue13 != null) {
						...
						if (previousValue13.getEdges().size() == o_cast.getEdges().size()) {
					...
					if (noChange13) {
						newState.getActivity_edges_Values().add(previousValue13);
					...
					// Then we compare the value of the field with the last stored value
					// If same value, we create no local state and we refer to the previous
					List<activitydiagramTrace.States.Activity_nodes_Value> localTrace14 = tracedObject
							.getNodesSequence();
					activitydiagramTrace.States.Activity_nodes_Value previousValue14 = null;
					...
					if (previousValue14 != null) {
						...
						if (previousValue14.getNodes().size() == o_cast.getNodes().size()) {

							java.util.Iterator<activitydiagram.ActivityNode> it = o_cast.getNodes().iterator();
							for (activitydiagramTrace.States.activitydiagram.TracedActivityNode aPreviousValue : previousValue14
									.getNodes()) {
					...
					if (noChange14) {
						newState.getActivity_nodes_Values().add(previousValue14);
					...
					// Then we compare the value of the field with the last stored value
					// If same value, we create no local state and we refer to the previous
					List<activitydiagramTrace.States.Activity_trace_Value> localTrace15 = tracedObject
							.getTraceSequence();
					activitydiagramTrace.States.Activity_trace_Value previousValue15 = null;
					if (!localTrace15.isEmpty())
						previousValue15 = localTrace15.get(localTrace15.size() - 1);

					storeAsTracedObject(o_cast.getTrace());

					activitydiagramTrace.States.activitydiagram.TracedTrace content9 = null;
					if (o_cast.getTrace() != null)
						content9 = ((activitydiagramTrace.States.activitydiagram.TracedTrace) exeToTraced.get(o_cast
								.getTrace()));

					boolean noChange15 = previousValue15 != null && previousValue15.getTrace() == content9;
					...
					// Then we compare the value of the field with the last stored value
					// If same value, we create no local state and we refer to the previous
					List<activitydiagramTrace.States.Activity_locals_Value> localTrace16 = tracedObject
							.getLocalsSequence();
					activitydiagramTrace.States.Activity_locals_Value previousValue16 = null;
					...
					if (previousValue16 != null) {
						...
						if (previousValue16.getLocals().size() == o_cast.getLocals().size()) {

							java.util.Iterator<activitydiagram.Variable> it = o_cast.getLocals().iterator();
							for (activitydiagramTrace.States.activitydiagram.TracedVariable aPreviousValue : previousValue16
									.getLocals()) {
					...
					if (noChange16) {
						newState.getActivity_locals_Values().add(previousValue16);
					...
					// Then we compare the value of the field with the last stored value
					// If same value, we create no local state and we refer to the previous
					List<activitydiagramTrace.States.Activity_inputs_Value> localTrace17 = tracedObject
							.getInputsSequence();
					activitydiagramTrace.States.Activity_inputs_Value previousValue17 = null;
					...
					if (previousValue17 != null) {
						...
						if (previousValue17.getInputs().size() == o_cast.getInputs().size()) {

							java.util.Iterator<activitydiagram.Variable> it = o_cast.getInputs().iterator();
							for (activitydiagramTrace.States.activitydiagram.TracedVariable aPreviousValue : previousValue17
									.getInputs()) {
								activitydiagram.Variable aCurrentValue = it.next();
								if (aPreviousValue != exeToTraced.get(aCurrentValue)) {
									noChange17 = false;
					...
					if (noChange17) {
						newState.getActivity_inputs_Values().add(previousValue17);
					...
					// Then we compare the value of the field with the last stored value
					// If same value, we create no local state and we refer to the previous
					List<activitydiagramTrace.States.NamedElement_name_Value> localTrace18 = tracedObject
							.getNameSequence();
					activitydiagramTrace.States.NamedElement_name_Value previousValue18 = null;
					if (!localTrace18.isEmpty())
						previousValue18 = localTrace18.get(localTrace18.size() - 1);

					java.lang.String content10 = o_cast.getName();

					boolean noChange18 = previousValue18 != null && previousValue18.getName() == content10;

					if (noChange18) {
						newState.getNamedElement_name_Values().add(previousValue18);
				...
				if (o instanceof activitydiagram.IntegerComparisonExpression) {
					...
					// Then we compare the value of the field with the last stored value
					// If same value, we create no local state and we refer to the previous
					List<activitydiagramTrace.States.IntegerComparisonExpression_operator_Value> localTrace19 = tracedObject
							.getOperatorSequence();
					activitydiagramTrace.States.IntegerComparisonExpression_operator_Value previousValue19 = null;
					if (!localTrace19.isEmpty())
						previousValue19 = localTrace19.get(localTrace19.size() - 1);

					activitydiagram.IntegerComparisonOperator content11 = o_cast.getOperator();

					boolean noChange19 = previousValue19 != null && previousValue19.getOperator() == content11;

					if (noChange19) {
						newState.getIntegerComparisonExpression_operator_Values().add(previousValue19);
						...
						newValue.setOperator(content11);
					...
					// Then we compare the value of the field with the last stored value
					// If same value, we create no local state and we refer to the previous
					List<activitydiagramTrace.States.IntegerExpression_operand2_Value> localTrace20 = tracedObject
							.getOperand2Sequence();
					activitydiagramTrace.States.IntegerExpression_operand2_Value previousValue20 = null;
					if (!localTrace20.isEmpty())
						previousValue20 = localTrace20.get(localTrace20.size() - 1);

					storeAsTracedObject(o_cast.getOperand2());

					activitydiagramTrace.States.activitydiagram.TracedIntegerVariable content12 = null;
					if (o_cast.getOperand2() != null)
						content12 = ((activitydiagramTrace.States.activitydiagram.TracedIntegerVariable) exeToTraced
								.get(o_cast.getOperand2()));

					boolean noChange20 = previousValue20 != null && previousValue20.getOperand2() == content12;

					if (noChange20) {
						newState.getIntegerExpression_operand2_Values().add(previousValue20);
						...
						newValue.setOperand2(content12);
					...
					// Then we compare the value of the field with the last stored value
					// If same value, we create no local state and we refer to the previous
					List<activitydiagramTrace.States.IntegerComparisonExpression_assignee_Value> localTrace21 = tracedObject
							.getAssigneeSequence();
					activitydiagramTrace.States.IntegerComparisonExpression_assignee_Value previousValue21 = null;
					if (!localTrace21.isEmpty())
						previousValue21 = localTrace21.get(localTrace21.size() - 1);

					storeAsTracedObject(o_cast.getAssignee());

					activitydiagramTrace.States.activitydiagram.TracedBooleanVariable content13 = null;
					if (o_cast.getAssignee() != null)
						content13 = ((activitydiagramTrace.States.activitydiagram.TracedBooleanVariable) exeToTraced
								.get(o_cast.getAssignee()));

					boolean noChange21 = previousValue21 != null && previousValue21.getAssignee() == content13;

					if (noChange21) {
						newState.getIntegerComparisonExpression_assignee_Values().add(previousValue21);
						...
						newValue.setAssignee(content13);
					...
					// Then we compare the value of the field with the last stored value
					// If same value, we create no local state and we refer to the previous
					List<activitydiagramTrace.States.IntegerExpression_operand1_Value> localTrace22 = tracedObject
							.getOperand1Sequence();
					activitydiagramTrace.States.IntegerExpression_operand1_Value previousValue22 = null;
					if (!localTrace22.isEmpty())
						previousValue22 = localTrace22.get(localTrace22.size() - 1);

					storeAsTracedObject(o_cast.getOperand1());

					activitydiagramTrace.States.activitydiagram.TracedIntegerVariable content14 = null;
					if (o_cast.getOperand1() != null)
						content14 = ((activitydiagramTrace.States.activitydiagram.TracedIntegerVariable) exeToTraced
								.get(o_cast.getOperand1()));

					boolean noChange22 = previousValue22 != null && previousValue22.getOperand1() == content14;

					if (noChange22) {
						newState.getIntegerExpression_operand1_Values().add(previousValue22);
						...
						newValue.setOperand1(content14);
				...
				if (o instanceof activitydiagram.Input) {
					...
					// Then we compare the value of the field with the last stored value
					// If same value, we create no local state and we refer to the previous
					List<activitydiagramTrace.States.Input_inputValues_Value> localTrace23 = tracedObject
							.getInputValuesSequence();
					activitydiagramTrace.States.Input_inputValues_Value previousValue23 = null;
					...
					boolean noChange23 = true;
					if (previousValue23 != null) {

						if (previousValue23.getInputValues().size() == o_cast.getInputValues().size()) {

							java.util.Iterator<activitydiagram.InputValue> it = o_cast.getInputValues().iterator();
							for (activitydiagramTrace.States.activitydiagram.TracedInputValue aPreviousValue : previousValue23
									.getInputValues()) {
					...
					if (noChange23) {
						newState.getInput_inputValues_Values().add(previousValue23);
				...
				if (o instanceof activitydiagram.ControlToken) {
					...
					// Then we compare the value of the field with the last stored value
					// If same value, we create no local state and we refer to the previous
					List<activitydiagramTrace.States.Token_holder_Value> localTrace24 = tracedObject
							.getHolderSequence();
					activitydiagramTrace.States.Token_holder_Value previousValue24 = null;
					if (!localTrace24.isEmpty())
						previousValue24 = localTrace24.get(localTrace24.size() - 1);

					storeAsTracedObject(o_cast.getHolder());

					activitydiagramTrace.States.activitydiagram.TracedActivityNode content15 = null;
					if (o_cast.getHolder() != null)
						content15 = ((activitydiagramTrace.States.activitydiagram.TracedActivityNode) exeToTraced
								.get(o_cast.getHolder()));

					boolean noChange24 = previousValue24 != null && previousValue24.getHolder() == content15;

					if (noChange24) {
						newState.getToken_holder_Values().add(previousValue24);
				...
				if (o instanceof activitydiagram.ForkedToken) {
					...
					// Then we compare the value of the field with the last stored value
					// If same value, we create no local state and we refer to the previous
					List<activitydiagramTrace.States.ForkedToken_baseToken_Value> localTrace25 = tracedObject
							.getBaseTokenSequence();
					activitydiagramTrace.States.ForkedToken_baseToken_Value previousValue25 = null;
					if (!localTrace25.isEmpty())
						previousValue25 = localTrace25.get(localTrace25.size() - 1);

					storeAsTracedObject(o_cast.getBaseToken());

					activitydiagramTrace.States.activitydiagram.TracedToken content16 = null;
					if (o_cast.getBaseToken() != null)
						content16 = ((activitydiagramTrace.States.activitydiagram.TracedToken) exeToTraced.get(o_cast
								.getBaseToken()));

					boolean noChange25 = previousValue25 != null && previousValue25.getBaseToken() == content16;

					if (noChange25) {
						newState.getForkedToken_baseToken_Values().add(previousValue25);
						...
						newValue.setBaseToken(content16);
					...
					// Then we compare the value of the field with the last stored value
					// If same value, we create no local state and we refer to the previous
					List<activitydiagramTrace.States.Token_holder_Value> localTrace26 = tracedObject
							.getHolderSequence();
					activitydiagramTrace.States.Token_holder_Value previousValue26 = null;
					if (!localTrace26.isEmpty())
						previousValue26 = localTrace26.get(localTrace26.size() - 1);

					storeAsTracedObject(o_cast.getHolder());

					activitydiagramTrace.States.activitydiagram.TracedActivityNode content17 = null;
					if (o_cast.getHolder() != null)
						content17 = ((activitydiagramTrace.States.activitydiagram.TracedActivityNode) exeToTraced
								.get(o_cast.getHolder()));

					boolean noChange26 = previousValue26 != null && previousValue26.getHolder() == content17;

					if (noChange26) {
						newState.getToken_holder_Values().add(previousValue26);
					...
					// Then we compare the value of the field with the last stored value
					// If same value, we create no local state and we refer to the previous
					List<activitydiagramTrace.States.ForkedToken_remainingOffersCount_Value> localTrace27 = tracedObject
							.getRemainingOffersCountSequence();
					activitydiagramTrace.States.ForkedToken_remainingOffersCount_Value previousValue27 = null;
					if (!localTrace27.isEmpty())
						previousValue27 = localTrace27.get(localTrace27.size() - 1);

					int content18 = o_cast.getRemainingOffersCount();

					boolean noChange27 = previousValue27 != null
							&& previousValue27.getRemainingOffersCount() == content18;

					if (noChange27) {
						newState.getForkedToken_remainingOffersCount_Values().add(previousValue27);
						...
						newValue.setRemainingOffersCount(content18);
				...
				if (o instanceof activitydiagram.BooleanValue) {
					...
					// Then we compare the value of the field with the last stored value
					// If same value, we create no local state and we refer to the previous
					List<activitydiagramTrace.States.BooleanValue_value_Value> localTrace28 = tracedObject
							.getValueSequence();
					activitydiagramTrace.States.BooleanValue_value_Value previousValue28 = null;
					if (!localTrace28.isEmpty())
						previousValue28 = localTrace28.get(localTrace28.size() - 1);

					boolean content19 = o_cast.isValue();

					boolean noChange28 = previousValue28 != null && previousValue28.isValue() == content19;

					if (noChange28) {
						newState.getBooleanValue_value_Values().add(previousValue28);
				...
				if (o instanceof activitydiagram.BooleanBinaryExpression) {
					...
					// Then we compare the value of the field with the last stored value
					// If same value, we create no local state and we refer to the previous
					List<activitydiagramTrace.States.BooleanBinaryExpression_operand1_Value> localTrace29 = tracedObject
							.getOperand1Sequence();
					activitydiagramTrace.States.BooleanBinaryExpression_operand1_Value previousValue29 = null;
					if (!localTrace29.isEmpty())
						previousValue29 = localTrace29.get(localTrace29.size() - 1);

					storeAsTracedObject(o_cast.getOperand1());

					activitydiagramTrace.States.activitydiagram.TracedBooleanVariable content20 = null;
					if (o_cast.getOperand1() != null)
						content20 = ((activitydiagramTrace.States.activitydiagram.TracedBooleanVariable) exeToTraced
								.get(o_cast.getOperand1()));

					boolean noChange29 = previousValue29 != null && previousValue29.getOperand1() == content20;

					if (noChange29) {
						newState.getBooleanBinaryExpression_operand1_Values().add(previousValue29);
					...
					// Then we compare the value of the field with the last stored value
					// If same value, we create no local state and we refer to the previous
					List<activitydiagramTrace.States.BooleanBinaryExpression_operator_Value> localTrace30 = tracedObject
							.getOperatorSequence();
					activitydiagramTrace.States.BooleanBinaryExpression_operator_Value previousValue30 = null;
					if (!localTrace30.isEmpty())
						previousValue30 = localTrace30.get(localTrace30.size() - 1);

					activitydiagram.BooleanBinaryOperator content21 = o_cast.getOperator();

					boolean noChange30 = previousValue30 != null && previousValue30.getOperator() == content21;

					if (noChange30) {
						newState.getBooleanBinaryExpression_operator_Values().add(previousValue30);
					...
					// Then we compare the value of the field with the last stored value
					// If same value, we create no local state and we refer to the previous
					List<activitydiagramTrace.States.BooleanExpression_assignee_Value> localTrace31 = tracedObject
							.getAssigneeSequence();
					activitydiagramTrace.States.BooleanExpression_assignee_Value previousValue31 = null;
					...
					boolean noChange31 = previousValue31 != null && previousValue31.getAssignee() == content22;

					if (noChange31) {
						newState.getBooleanExpression_assignee_Values().add(previousValue31);
					...
					// Then we compare the value of the field with the last stored value
					// If same value, we create no local state and we refer to the previous
					List<activitydiagramTrace.States.BooleanBinaryExpression_operand2_Value> localTrace32 = tracedObject
							.getOperand2Sequence();
					activitydiagramTrace.States.BooleanBinaryExpression_operand2_Value previousValue32 = null;
					...
					boolean noChange32 = previousValue32 != null && previousValue32.getOperand2() == content23;

					if (noChange32) {
						newState.getBooleanBinaryExpression_operand2_Values().add(previousValue32);
						...
						newValue.setOperand2(content23);
				...
				if (o instanceof activitydiagram.BooleanUnaryExpression) {
					...
					// Then we compare the value of the field with the last stored value
					// If same value, we create no local state and we refer to the previous
					List<activitydiagramTrace.States.BooleanUnaryExpression_operand_Value> localTrace33 = tracedObject
							.getOperandSequence();
					activitydiagramTrace.States.BooleanUnaryExpression_operand_Value previousValue33 = null;
					if (!localTrace33.isEmpty())
						previousValue33 = localTrace33.get(localTrace33.size() - 1);

					storeAsTracedObject(o_cast.getOperand());

					activitydiagramTrace.States.activitydiagram.TracedBooleanVariable content24 = null;
					if (o_cast.getOperand() != null)
						content24 = ((activitydiagramTrace.States.activitydiagram.TracedBooleanVariable) exeToTraced
								.get(o_cast.getOperand()));

					boolean noChange33 = previousValue33 != null && previousValue33.getOperand() == content24;

					if (noChange33) {
						newState.getBooleanUnaryExpression_operand_Values().add(previousValue33);
					...
					// Then we compare the value of the field with the last stored value
					// If same value, we create no local state and we refer to the previous
					List<activitydiagramTrace.States.BooleanUnaryExpression_operator_Value> localTrace34 = tracedObject
							.getOperatorSequence();
					activitydiagramTrace.States.BooleanUnaryExpression_operator_Value previousValue34 = null;
					if (!localTrace34.isEmpty())
						previousValue34 = localTrace34.get(localTrace34.size() - 1);

					activitydiagram.BooleanUnaryOperator content25 = o_cast.getOperator();

					boolean noChange34 = previousValue34 != null && previousValue34.getOperator() == content25;

					if (noChange34) {
						newState.getBooleanUnaryExpression_operator_Values().add(previousValue34);
						...
						newValue.setOperator(content25);
					...
					// Then we compare the value of the field with the last stored value
					// If same value, we create no local state and we refer to the previous
					List<activitydiagramTrace.States.BooleanExpression_assignee_Value> localTrace35 = tracedObject
							.getAssigneeSequence();
					activitydiagramTrace.States.BooleanExpression_assignee_Value previousValue35 = null;
					if (!localTrace35.isEmpty())
						previousValue35 = localTrace35.get(localTrace35.size() - 1);

					storeAsTracedObject(o_cast.getAssignee());

					activitydiagramTrace.States.activitydiagram.TracedBooleanVariable content26 = null;
					if (o_cast.getAssignee() != null)
						content26 = ((activitydiagramTrace.States.activitydiagram.TracedBooleanVariable) exeToTraced
								.get(o_cast.getAssignee()));

					boolean noChange35 = previousValue35 != null && previousValue35.getAssignee() == content26;

					if (noChange35) {
						newState.getBooleanExpression_assignee_Values().add(previousValue35);
						...
						newValue.setAssignee(content26);
				...
				if (o instanceof activitydiagram.InputValue) {
					...
					// Then we compare the value of the field with the last stored value
					// If same value, we create no local state and we refer to the previous
					List<activitydiagramTrace.States.InputValue_value_Value> localTrace36 = tracedObject
							.getValueSequence();
					activitydiagramTrace.States.InputValue_value_Value previousValue36 = null;
					if (!localTrace36.isEmpty())
						previousValue36 = localTrace36.get(localTrace36.size() - 1);

					storeAsTracedObject(o_cast.getValue());

					activitydiagramTrace.States.activitydiagram.TracedValue content27 = null;
					if (o_cast.getValue() != null)
						content27 = ((activitydiagramTrace.States.activitydiagram.TracedValue) exeToTraced.get(o_cast
								.getValue()));

					boolean noChange36 = previousValue36 != null && previousValue36.getValue() == content27;

					if (noChange36) {
						newState.getInputValue_value_Values().add(previousValue36);
					...
					// Then we compare the value of the field with the last stored value
					// If same value, we create no local state and we refer to the previous
					List<activitydiagramTrace.States.InputValue_variable_Value> localTrace37 = tracedObject
							.getVariableSequence();
					activitydiagramTrace.States.InputValue_variable_Value previousValue37 = null;
					if (!localTrace37.isEmpty())
						previousValue37 = localTrace37.get(localTrace37.size() - 1);

					storeAsTracedObject(o_cast.getVariable());

					activitydiagramTrace.States.activitydiagram.TracedVariable content28 = null;
					if (o_cast.getVariable() != null)
						content28 = ((activitydiagramTrace.States.activitydiagram.TracedVariable) exeToTraced
								.get(o_cast.getVariable()));

					boolean noChange37 = previousValue37 != null && previousValue37.getVariable() == content28;

					if (noChange37) {
						newState.getInputValue_variable_Values().add(previousValue37);
						...
						newValue.setVariable(content28);
				...
				if (o instanceof activitydiagram.OpaqueAction) {
					...
					// Then we compare the value of the field with the last stored value
					// If same value, we create no local state and we refer to the previous
					List<activitydiagramTrace.States.OpaqueAction_expressions_Value> localTrace39 = tracedObject
							.getExpressionsSequence();
					activitydiagramTrace.States.OpaqueAction_expressions_Value previousValue39 = null;
					...
					if (previousValue39 != null) {
						...
						if (previousValue39.getExpressions().size() == o_cast.getExpressions().size()) {

							java.util.Iterator<activitydiagram.Expression> it = o_cast.getExpressions().iterator();
							for (activitydiagramTrace.States.activitydiagram.TracedExpression aPreviousValue : previousValue39
									.getExpressions()) {
					...
					if (noChange39) {
						newState.getOpaqueAction_expressions_Values().add(previousValue39);
					...
					// Then we compare the value of the field with the last stored value
					// If same value, we create no local state and we refer to the previous
					List<activitydiagramTrace.States.ActivityNode_incoming_Value> localTrace40 = tracedObject
							.getIncomingSequence();
					activitydiagramTrace.States.ActivityNode_incoming_Value previousValue40 = null;
					...
					if (previousValue40 != null) {
						...
						if (previousValue40.getIncoming().size() == o_cast.getIncoming().size()) {
					...
					if (noChange40) {
						newState.getActivityNode_incoming_Values().add(previousValue40);
					...
					boolean content29 = o_cast.isRunning();

					boolean noChange41 = previousValue41 != null && previousValue41.isRunning() == content29;
					...
					// Then we compare the value of the field with the last stored value
					// If same value, we create no local state and we refer to the previous
					List<activitydiagramTrace.States.ActivityNode_activity_Value> localTrace42 = tracedObject
							.getActivitySequence();
					activitydiagramTrace.States.ActivityNode_activity_Value previousValue42 = null;
					...
					boolean noChange42 = previousValue42 != null && previousValue42.getActivity() == content30;

					if (noChange42) {
						newState.getActivityNode_activity_Values().add(previousValue42);
					...
					// Then we compare the value of the field with the last stored value
					// If same value, we create no local state and we refer to the previous
					List<activitydiagramTrace.States.NamedElement_name_Value> localTrace43 = tracedObject
							.getNameSequence();
					activitydiagramTrace.States.NamedElement_name_Value previousValue43 = null;
					if (!localTrace43.isEmpty())
						previousValue43 = localTrace43.get(localTrace43.size() - 1);

					java.lang.String content31 = o_cast.getName();

					boolean noChange43 = previousValue43 != null && previousValue43.getName() == content31;

					if (noChange43) {
						newState.getNamedElement_name_Values().add(previousValue43);
					...
					// Then we compare the value of the field with the last stored value
					// If same value, we create no local state and we refer to the previous
					List<activitydiagramTrace.States.ActivityNode_outgoing_Value> localTrace44 = tracedObject
							.getOutgoingSequence();
					activitydiagramTrace.States.ActivityNode_outgoing_Value previousValue44 = null;
					...
					boolean noChange44 = true;
					if (previousValue44 != null) {

						if (previousValue44.getOutgoing().size() == o_cast.getOutgoing().size()) {

							java.util.Iterator<activitydiagram.ActivityEdge> it = o_cast.getOutgoing().iterator();
							for (activitydiagramTrace.States.activitydiagram.TracedActivityEdge aPreviousValue : previousValue44
									.getOutgoing()) {
								activitydiagram.ActivityEdge aCurrentValue = it.next();
								if (aPreviousValue != exeToTraced.get(aCurrentValue)) {
									noChange44 = false;
					...
					if (noChange44) {
						newState.getActivityNode_outgoing_Values().add(previousValue44);
				...
				if (o instanceof activitydiagram.IntegerVariable) {
					...
					// Then we compare the value of the field with the last stored value
					// If same value, we create no local state and we refer to the previous
					List<activitydiagramTrace.States.Variable_initialValue_Value> localTrace45 = tracedObject
							.getInitialValueSequence();
					activitydiagramTrace.States.Variable_initialValue_Value previousValue45 = null;
					if (!localTrace45.isEmpty())
						previousValue45 = localTrace45.get(localTrace45.size() - 1);

					storeAsTracedObject(o_cast.getInitialValue());

					activitydiagramTrace.States.activitydiagram.TracedValue content32 = null;
					if (o_cast.getInitialValue() != null)
						content32 = ((activitydiagramTrace.States.activitydiagram.TracedValue) exeToTraced.get(o_cast
								.getInitialValue()));

					boolean noChange45 = previousValue45 != null && previousValue45.getInitialValue() == content32;

					if (noChange45) {
						newState.getVariable_initialValue_Values().add(previousValue45);
						...
						newValue.setInitialValue(content32);
					...
					// Then we compare the value of the field with the last stored value
					// If same value, we create no local state and we refer to the previous
					List<activitydiagramTrace.States.Variable_currentValue_Value> localTrace46 = tracedObject
							.getCurrentValueSequence();
					activitydiagramTrace.States.Variable_currentValue_Value previousValue46 = null;
					if (!localTrace46.isEmpty())
						previousValue46 = localTrace46.get(localTrace46.size() - 1);

					storeAsTracedObject(o_cast.getCurrentValue());

					activitydiagramTrace.States.activitydiagram.TracedValue content33 = null;
					if (o_cast.getCurrentValue() != null)
						content33 = ((activitydiagramTrace.States.activitydiagram.TracedValue) exeToTraced.get(o_cast
								.getCurrentValue()));

					boolean noChange46 = previousValue46 != null && previousValue46.getCurrentValue() == content33;

					if (noChange46) {
						newState.getVariable_currentValue_Values().add(previousValue46);
						...
						newValue.setCurrentValue(content33);
					...
					// Then we compare the value of the field with the last stored value
					// If same value, we create no local state and we refer to the previous
					List<activitydiagramTrace.States.Variable_name_Value> localTrace47 = tracedObject.getNameSequence();
					activitydiagramTrace.States.Variable_name_Value previousValue47 = null;
					if (!localTrace47.isEmpty())
						previousValue47 = localTrace47.get(localTrace47.size() - 1);

					java.lang.String content34 = o_cast.getName();

					boolean noChange47 = previousValue47 != null && previousValue47.getName() == content34;

					if (noChange47) {
						newState.getVariable_name_Values().add(previousValue47);
				...
				if (o instanceof activitydiagram.DecisionNode) {
					...
					// Then we compare the value of the field with the last stored value
					// If same value, we create no local state and we refer to the previous
					List<activitydiagramTrace.States.ActivityNode_heldTokens_Value> localTrace48 = tracedObject
							.getHeldTokensSequence();
					activitydiagramTrace.States.ActivityNode_heldTokens_Value previousValue48 = null;
					...
					if (previousValue48 != null) {
						...
						if (previousValue48.getHeldTokens().size() == o_cast.getHeldTokens().size()) {

							java.util.Iterator<activitydiagram.Token> it = o_cast.getHeldTokens().iterator();
							for (activitydiagramTrace.States.activitydiagram.TracedToken aPreviousValue : previousValue48
									.getHeldTokens()) {
								activitydiagram.Token aCurrentValue = it.next();
								if (aPreviousValue != exeToTraced.get(aCurrentValue)) {
									noChange48 = false;
					...
					if (noChange48) {
						newState.getActivityNode_heldTokens_Values().add(previousValue48);
					...
					// Then we compare the value of the field with the last stored value
					// If same value, we create no local state and we refer to the previous
					List<activitydiagramTrace.States.ActivityNode_incoming_Value> localTrace49 = tracedObject
							.getIncomingSequence();
					activitydiagramTrace.States.ActivityNode_incoming_Value previousValue49 = null;
					...
					if (previousValue49 != null) {
						...
						if (previousValue49.getIncoming().size() == o_cast.getIncoming().size()) {
					...
					if (noChange49) {
						newState.getActivityNode_incoming_Values().add(previousValue49);
					...
					boolean noChange50 = previousValue50 != null && previousValue50.isRunning() == content35;
					...
					// Then we compare the value of the field with the last stored value
					// If same value, we create no local state and we refer to the previous
					List<activitydiagramTrace.States.ActivityNode_activity_Value> localTrace51 = tracedObject
							.getActivitySequence();
					activitydiagramTrace.States.ActivityNode_activity_Value previousValue51 = null;
					...
					boolean noChange51 = previousValue51 != null && previousValue51.getActivity() == content36;

					if (noChange51) {
						newState.getActivityNode_activity_Values().add(previousValue51);
					...
					// Then we compare the value of the field with the last stored value
					// If same value, we create no local state and we refer to the previous
					List<activitydiagramTrace.States.NamedElement_name_Value> localTrace52 = tracedObject
							.getNameSequence();
					activitydiagramTrace.States.NamedElement_name_Value previousValue52 = null;
					if (!localTrace52.isEmpty())
						previousValue52 = localTrace52.get(localTrace52.size() - 1);

					java.lang.String content37 = o_cast.getName();

					boolean noChange52 = previousValue52 != null && previousValue52.getName() == content37;

					if (noChange52) {
						newState.getNamedElement_name_Values().add(previousValue52);
					...
					// Then we compare the value of the field with the last stored value
					// If same value, we create no local state and we refer to the previous
					List<activitydiagramTrace.States.ActivityNode_outgoing_Value> localTrace53 = tracedObject
							.getOutgoingSequence();
					activitydiagramTrace.States.ActivityNode_outgoing_Value previousValue53 = null;
					...
					if (previousValue53 != null) {
						...
						if (previousValue53.getOutgoing().size() == o_cast.getOutgoing().size()) {

							java.util.Iterator<activitydiagram.ActivityEdge> it = o_cast.getOutgoing().iterator();
							for (activitydiagramTrace.States.activitydiagram.TracedActivityEdge aPreviousValue : previousValue53
									.getOutgoing()) {
								activitydiagram.ActivityEdge aCurrentValue = it.next();
								if (aPreviousValue != exeToTraced.get(aCurrentValue)) {
									noChange53 = false;
					...
					if (noChange53) {
						newState.getActivityNode_outgoing_Values().add(previousValue53);
				...
				if (o instanceof activitydiagram.MergeNode) {
					...
					// Then we compare the value of the field with the last stored value
					// If same value, we create no local state and we refer to the previous
					List<activitydiagramTrace.States.ActivityNode_heldTokens_Value> localTrace54 = tracedObject
							.getHeldTokensSequence();
					activitydiagramTrace.States.ActivityNode_heldTokens_Value previousValue54 = null;
					...
					if (previousValue54 != null) {
						...
						if (previousValue54.getHeldTokens().size() == o_cast.getHeldTokens().size()) {

							java.util.Iterator<activitydiagram.Token> it = o_cast.getHeldTokens().iterator();
							for (activitydiagramTrace.States.activitydiagram.TracedToken aPreviousValue : previousValue54
									.getHeldTokens()) {
								activitydiagram.Token aCurrentValue = it.next();
								if (aPreviousValue != exeToTraced.get(aCurrentValue)) {
									noChange54 = false;
					...
					if (noChange54) {
						newState.getActivityNode_heldTokens_Values().add(previousValue54);
					...
					// Then we compare the value of the field with the last stored value
					// If same value, we create no local state and we refer to the previous
					List<activitydiagramTrace.States.ActivityNode_incoming_Value> localTrace55 = tracedObject
							.getIncomingSequence();
					activitydiagramTrace.States.ActivityNode_incoming_Value previousValue55 = null;
					...
					if (previousValue55 != null) {
						...
						if (previousValue55.getIncoming().size() == o_cast.getIncoming().size()) {
					...
					if (noChange55) {
						newState.getActivityNode_incoming_Values().add(previousValue55);
					...
					boolean noChange56 = previousValue56 != null && previousValue56.isRunning() == content38;
					...
					// Then we compare the value of the field with the last stored value
					// If same value, we create no local state and we refer to the previous
					List<activitydiagramTrace.States.ActivityNode_activity_Value> localTrace57 = tracedObject
							.getActivitySequence();
					activitydiagramTrace.States.ActivityNode_activity_Value previousValue57 = null;
					...
					boolean noChange57 = previousValue57 != null && previousValue57.getActivity() == content39;

					if (noChange57) {
						newState.getActivityNode_activity_Values().add(previousValue57);
					...
					// Then we compare the value of the field with the last stored value
					// If same value, we create no local state and we refer to the previous
					List<activitydiagramTrace.States.NamedElement_name_Value> localTrace58 = tracedObject
							.getNameSequence();
					activitydiagramTrace.States.NamedElement_name_Value previousValue58 = null;
					if (!localTrace58.isEmpty())
						previousValue58 = localTrace58.get(localTrace58.size() - 1);

					java.lang.String content40 = o_cast.getName();

					boolean noChange58 = previousValue58 != null && previousValue58.getName() == content40;

					if (noChange58) {
						newState.getNamedElement_name_Values().add(previousValue58);
					...
					// Then we compare the value of the field with the last stored value
					// If same value, we create no local state and we refer to the previous
					List<activitydiagramTrace.States.ActivityNode_outgoing_Value> localTrace59 = tracedObject
							.getOutgoingSequence();
					activitydiagramTrace.States.ActivityNode_outgoing_Value previousValue59 = null;
					...
					if (previousValue59 != null) {
						...
						if (previousValue59.getOutgoing().size() == o_cast.getOutgoing().size()) {

							java.util.Iterator<activitydiagram.ActivityEdge> it = o_cast.getOutgoing().iterator();
							for (activitydiagramTrace.States.activitydiagram.TracedActivityEdge aPreviousValue : previousValue59
									.getOutgoing()) {
								activitydiagram.ActivityEdge aCurrentValue = it.next();
								if (aPreviousValue != exeToTraced.get(aCurrentValue)) {
									noChange59 = false;
					...
					if (noChange59) {
						newState.getActivityNode_outgoing_Values().add(previousValue59);
				...
				if (o instanceof activitydiagram.ControlFlow) {
					...
					// Then we compare the value of the field with the last stored value
					// If same value, we create no local state and we refer to the previous
					List<activitydiagramTrace.States.ControlFlow_guard_Value> localTrace60 = tracedObject
							.getGuardSequence();
					activitydiagramTrace.States.ControlFlow_guard_Value previousValue60 = null;
					if (!localTrace60.isEmpty())
						previousValue60 = localTrace60.get(localTrace60.size() - 1);

					storeAsTracedObject(o_cast.getGuard());

					activitydiagramTrace.States.activitydiagram.TracedBooleanVariable content41 = null;
					if (o_cast.getGuard() != null)
						content41 = ((activitydiagramTrace.States.activitydiagram.TracedBooleanVariable) exeToTraced
								.get(o_cast.getGuard()));

					boolean noChange60 = previousValue60 != null && previousValue60.getGuard() == content41;

					if (noChange60) {
						newState.getControlFlow_guard_Values().add(previousValue60);
					...
					// Then we compare the value of the field with the last stored value
					// If same value, we create no local state and we refer to the previous
					List<activitydiagramTrace.States.ActivityEdge_source_Value> localTrace61 = tracedObject
							.getSourceSequence();
					activitydiagramTrace.States.ActivityEdge_source_Value previousValue61 = null;
					if (!localTrace61.isEmpty())
						previousValue61 = localTrace61.get(localTrace61.size() - 1);

					storeAsTracedObject(o_cast.getSource());

					activitydiagramTrace.States.activitydiagram.TracedActivityNode content42 = null;
					if (o_cast.getSource() != null)
						content42 = ((activitydiagramTrace.States.activitydiagram.TracedActivityNode) exeToTraced
								.get(o_cast.getSource()));

					boolean noChange61 = previousValue61 != null && previousValue61.getSource() == content42;

					if (noChange61) {
						newState.getActivityEdge_source_Values().add(previousValue61);
					...
					// Then we compare the value of the field with the last stored value
					// If same value, we create no local state and we refer to the previous
					List<activitydiagramTrace.States.NamedElement_name_Value> localTrace62 = tracedObject
							.getNameSequence();
					activitydiagramTrace.States.NamedElement_name_Value previousValue62 = null;
					if (!localTrace62.isEmpty())
						previousValue62 = localTrace62.get(localTrace62.size() - 1);

					java.lang.String content43 = o_cast.getName();

					boolean noChange62 = previousValue62 != null && previousValue62.getName() == content43;

					if (noChange62) {
						newState.getNamedElement_name_Values().add(previousValue62);
					...
					// Then we compare the value of the field with the last stored value
					// If same value, we create no local state and we refer to the previous
					List<activitydiagramTrace.States.ActivityEdge_target_Value> localTrace63 = tracedObject
							.getTargetSequence();
					activitydiagramTrace.States.ActivityEdge_target_Value previousValue63 = null;
					if (!localTrace63.isEmpty())
						previousValue63 = localTrace63.get(localTrace63.size() - 1);

					storeAsTracedObject(o_cast.getTarget());

					activitydiagramTrace.States.activitydiagram.TracedActivityNode content44 = null;
					if (o_cast.getTarget() != null)
						content44 = ((activitydiagramTrace.States.activitydiagram.TracedActivityNode) exeToTraced
								.get(o_cast.getTarget()));

					boolean noChange63 = previousValue63 != null && previousValue63.getTarget() == content44;

					if (noChange63) {
						newState.getActivityEdge_target_Values().add(previousValue63);
					...
					// Then we compare the value of the field with the last stored value
					// If same value, we create no local state and we refer to the previous
					List<activitydiagramTrace.States.ActivityEdge_offers_Value> localTrace64 = tracedObject
							.getOffersSequence();
					activitydiagramTrace.States.ActivityEdge_offers_Value previousValue64 = null;
					...
					if (previousValue64 != null) {
						...
						if (previousValue64.getOffers().size() == o_cast.getOffers().size()) {

							java.util.Iterator<activitydiagram.Offer> it = o_cast.getOffers().iterator();
							for (activitydiagramTrace.States.activitydiagram.TracedOffer aPreviousValue : previousValue64
									.getOffers()) {
					...
					if (noChange64) {
						newState.getActivityEdge_offers_Values().add(previousValue64);
				...
				if (o instanceof activitydiagram.Trace) {
					...
					// Then we compare the value of the field with the last stored value
					// If same value, we create no local state and we refer to the previous
					List<activitydiagramTrace.States.Trace_executedNodes_Value> localTrace65 = tracedObject
							.getExecutedNodesSequence();
					activitydiagramTrace.States.Trace_executedNodes_Value previousValue65 = null;
					...
					boolean noChange65 = true;
					if (previousValue65 != null) {

						if (previousValue65.getExecutedNodes().size() == o_cast.getExecutedNodes().size()) {

							java.util.Iterator<activitydiagram.ActivityNode> it = o_cast.getExecutedNodes().iterator();
							for (activitydiagramTrace.States.activitydiagram.TracedActivityNode aPreviousValue : previousValue65
									.getExecutedNodes()) {
								activitydiagram.ActivityNode aCurrentValue = it.next();
								if (aPreviousValue != exeToTraced.get(aCurrentValue)) {
									noChange65 = false;
					...
					if (noChange65) {
						newState.getTrace_executedNodes_Values().add(previousValue65);
				...
				if (o instanceof activitydiagram.InitialNode) {
					...
					// Then we compare the value of the field with the last stored value
					// If same value, we create no local state and we refer to the previous
					List<activitydiagramTrace.States.ActivityNode_heldTokens_Value> localTrace66 = tracedObject
							.getHeldTokensSequence();
					activitydiagramTrace.States.ActivityNode_heldTokens_Value previousValue66 = null;
					...
					boolean noChange66 = true;
					if (previousValue66 != null) {

						if (previousValue66.getHeldTokens().size() == o_cast.getHeldTokens().size()) {

							java.util.Iterator<activitydiagram.Token> it = o_cast.getHeldTokens().iterator();
							for (activitydiagramTrace.States.activitydiagram.TracedToken aPreviousValue : previousValue66
									.getHeldTokens()) {
								activitydiagram.Token aCurrentValue = it.next();
								if (aPreviousValue != exeToTraced.get(aCurrentValue)) {
									noChange66 = false;
					...
					if (noChange66) {
						newState.getActivityNode_heldTokens_Values().add(previousValue66);
					...
					// Then we compare the value of the field with the last stored value
					// If same value, we create no local state and we refer to the previous
					List<activitydiagramTrace.States.ActivityNode_incoming_Value> localTrace67 = tracedObject
							.getIncomingSequence();
					activitydiagramTrace.States.ActivityNode_incoming_Value previousValue67 = null;
					...
					boolean noChange67 = true;
					if (previousValue67 != null) {

						if (previousValue67.getIncoming().size() == o_cast.getIncoming().size()) {

							java.util.Iterator<activitydiagram.ActivityEdge> it = o_cast.getIncoming().iterator();
							for (activitydiagramTrace.States.activitydiagram.TracedActivityEdge aPreviousValue : previousValue67
									.getIncoming()) {
								activitydiagram.ActivityEdge aCurrentValue = it.next();
								if (aPreviousValue != exeToTraced.get(aCurrentValue)) {
									noChange67 = false;
					...
					if (noChange67) {
						newState.getActivityNode_incoming_Values().add(previousValue67);
					...
					// Then we compare the value of the field with the last stored value
					// If same value, we create no local state and we refer to the previous
					List<activitydiagramTrace.States.ActivityNode_running_Value> localTrace68 = tracedObject
							.getRunningSequence();
					activitydiagramTrace.States.ActivityNode_running_Value previousValue68 = null;
					if (!localTrace68.isEmpty())
						previousValue68 = localTrace68.get(localTrace68.size() - 1);

					boolean content45 = o_cast.isRunning();

					boolean noChange68 = previousValue68 != null && previousValue68.isRunning() == content45;

					if (noChange68) {
						newState.getActivityNode_running_Values().add(previousValue68);
					...
					// Then we compare the value of the field with the last stored value
					// If same value, we create no local state and we refer to the previous
					List<activitydiagramTrace.States.ActivityNode_activity_Value> localTrace69 = tracedObject
							.getActivitySequence();
					activitydiagramTrace.States.ActivityNode_activity_Value previousValue69 = null;
					if (!localTrace69.isEmpty())
						previousValue69 = localTrace69.get(localTrace69.size() - 1);

					storeAsTracedObject(o_cast.getActivity());

					activitydiagramTrace.States.activitydiagram.TracedActivity content46 = null;
					if (o_cast.getActivity() != null)
						content46 = ((activitydiagramTrace.States.activitydiagram.TracedActivity) exeToTraced
								.get(o_cast.getActivity()));

					boolean noChange69 = previousValue69 != null && previousValue69.getActivity() == content46;

					if (noChange69) {
						newState.getActivityNode_activity_Values().add(previousValue69);
						...
						newValue.setActivity(content46);
					...
					// Then we compare the value of the field with the last stored value
					// If same value, we create no local state and we refer to the previous
					List<activitydiagramTrace.States.NamedElement_name_Value> localTrace70 = tracedObject
							.getNameSequence();
					activitydiagramTrace.States.NamedElement_name_Value previousValue70 = null;
					if (!localTrace70.isEmpty())
						previousValue70 = localTrace70.get(localTrace70.size() - 1);

					java.lang.String content47 = o_cast.getName();

					boolean noChange70 = previousValue70 != null && previousValue70.getName() == content47;

					if (noChange70) {
						newState.getNamedElement_name_Values().add(previousValue70);
					...
					// Then we compare the value of the field with the last stored value
					// If same value, we create no local state and we refer to the previous
					List<activitydiagramTrace.States.ActivityNode_outgoing_Value> localTrace71 = tracedObject
							.getOutgoingSequence();
					activitydiagramTrace.States.ActivityNode_outgoing_Value previousValue71 = null;
					...
					boolean noChange71 = true;
					if (previousValue71 != null) {

						if (previousValue71.getOutgoing().size() == o_cast.getOutgoing().size()) {

							java.util.Iterator<activitydiagram.ActivityEdge> it = o_cast.getOutgoing().iterator();
							for (activitydiagramTrace.States.activitydiagram.TracedActivityEdge aPreviousValue : previousValue71
									.getOutgoing()) {
								activitydiagram.ActivityEdge aCurrentValue = it.next();
								if (aPreviousValue != exeToTraced.get(aCurrentValue)) {
									noChange71 = false;
					...
					if (noChange71) {
						newState.getActivityNode_outgoing_Values().add(previousValue71);
				...
				if (o instanceof activitydiagram.BooleanVariable) {
					...
					// Then we compare the value of the field with the last stored value
					// If same value, we create no local state and we refer to the previous
					List<activitydiagramTrace.States.Variable_initialValue_Value> localTrace72 = tracedObject
							.getInitialValueSequence();
					activitydiagramTrace.States.Variable_initialValue_Value previousValue72 = null;
					...
					boolean noChange72 = previousValue72 != null && previousValue72.getInitialValue() == content48;

					if (noChange72) {
						newState.getVariable_initialValue_Values().add(previousValue72);
					...
					// Then we compare the value of the field with the last stored value
					// If same value, we create no local state and we refer to the previous
					List<activitydiagramTrace.States.Variable_currentValue_Value> localTrace73 = tracedObject
							.getCurrentValueSequence();
					activitydiagramTrace.States.Variable_currentValue_Value previousValue73 = null;
					if (!localTrace73.isEmpty())
						previousValue73 = localTrace73.get(localTrace73.size() - 1);

					storeAsTracedObject(o_cast.getCurrentValue());

					activitydiagramTrace.States.activitydiagram.TracedValue content49 = null;
					if (o_cast.getCurrentValue() != null)
						content49 = ((activitydiagramTrace.States.activitydiagram.TracedValue) exeToTraced.get(o_cast
								.getCurrentValue()));

					boolean noChange73 = previousValue73 != null && previousValue73.getCurrentValue() == content49;

					if (noChange73) {
						newState.getVariable_currentValue_Values().add(previousValue73);
						...
						newValue.setCurrentValue(content49);
					...
					// Then we compare the value of the field with the last stored value
					// If same value, we create no local state and we refer to the previous
					List<activitydiagramTrace.States.Variable_name_Value> localTrace74 = tracedObject.getNameSequence();
					activitydiagramTrace.States.Variable_name_Value previousValue74 = null;
					if (!localTrace74.isEmpty())
						previousValue74 = localTrace74.get(localTrace74.size() - 1);

					java.lang.String content50 = o_cast.getName();

					boolean noChange74 = previousValue74 != null && previousValue74.getName() == content50;

					if (noChange74) {
						newState.getVariable_name_Values().add(previousValue74);
				...
				if (o instanceof activitydiagram.ForkNode) {
					...
					// Then we compare the value of the field with the last stored value
					// If same value, we create no local state and we refer to the previous
					List<activitydiagramTrace.States.ActivityNode_heldTokens_Value> localTrace75 = tracedObject
							.getHeldTokensSequence();
					activitydiagramTrace.States.ActivityNode_heldTokens_Value previousValue75 = null;
					...
					boolean noChange75 = true;
					if (previousValue75 != null) {

						if (previousValue75.getHeldTokens().size() == o_cast.getHeldTokens().size()) {

							java.util.Iterator<activitydiagram.Token> it = o_cast.getHeldTokens().iterator();
							for (activitydiagramTrace.States.activitydiagram.TracedToken aPreviousValue : previousValue75
									.getHeldTokens()) {
								activitydiagram.Token aCurrentValue = it.next();
								if (aPreviousValue != exeToTraced.get(aCurrentValue)) {
									noChange75 = false;
					...
					if (noChange75) {
						newState.getActivityNode_heldTokens_Values().add(previousValue75);
					...
					// Then we compare the value of the field with the last stored value
					// If same value, we create no local state and we refer to the previous
					List<activitydiagramTrace.States.ActivityNode_incoming_Value> localTrace76 = tracedObject
							.getIncomingSequence();
					activitydiagramTrace.States.ActivityNode_incoming_Value previousValue76 = null;
					...
					if (previousValue76 != null) {
						...
						if (previousValue76.getIncoming().size() == o_cast.getIncoming().size()) {

							java.util.Iterator<activitydiagram.ActivityEdge> it = o_cast.getIncoming().iterator();
							for (activitydiagramTrace.States.activitydiagram.TracedActivityEdge aPreviousValue : previousValue76
									.getIncoming()) {
								activitydiagram.ActivityEdge aCurrentValue = it.next();
								if (aPreviousValue != exeToTraced.get(aCurrentValue)) {
									noChange76 = false;
					...
					if (noChange76) {
						newState.getActivityNode_incoming_Values().add(previousValue76);
					...
					// Then we compare the value of the field with the last stored value
					// If same value, we create no local state and we refer to the previous
					List<activitydiagramTrace.States.ActivityNode_running_Value> localTrace77 = tracedObject
							.getRunningSequence();
					activitydiagramTrace.States.ActivityNode_running_Value previousValue77 = null;
					if (!localTrace77.isEmpty())
						previousValue77 = localTrace77.get(localTrace77.size() - 1);

					boolean content51 = o_cast.isRunning();

					boolean noChange77 = previousValue77 != null && previousValue77.isRunning() == content51;

					if (noChange77) {
						newState.getActivityNode_running_Values().add(previousValue77);
					...
					// Then we compare the value of the field with the last stored value
					// If same value, we create no local state and we refer to the previous
					List<activitydiagramTrace.States.ActivityNode_activity_Value> localTrace78 = tracedObject
							.getActivitySequence();
					activitydiagramTrace.States.ActivityNode_activity_Value previousValue78 = null;
					...
					boolean noChange78 = previousValue78 != null && previousValue78.getActivity() == content52;

					if (noChange78) {
						newState.getActivityNode_activity_Values().add(previousValue78);
					...
					// Then we compare the value of the field with the last stored value
					// If same value, we create no local state and we refer to the previous
					List<activitydiagramTrace.States.NamedElement_name_Value> localTrace79 = tracedObject
							.getNameSequence();
					activitydiagramTrace.States.NamedElement_name_Value previousValue79 = null;
					if (!localTrace79.isEmpty())
						previousValue79 = localTrace79.get(localTrace79.size() - 1);

					java.lang.String content53 = o_cast.getName();

					boolean noChange79 = previousValue79 != null && previousValue79.getName() == content53;

					if (noChange79) {
						newState.getNamedElement_name_Values().add(previousValue79);
					...
					// Then we compare the value of the field with the last stored value
					// If same value, we create no local state and we refer to the previous
					List<activitydiagramTrace.States.ActivityNode_outgoing_Value> localTrace80 = tracedObject
							.getOutgoingSequence();
					activitydiagramTrace.States.ActivityNode_outgoing_Value previousValue80 = null;
					...
					boolean noChange80 = true;
					if (previousValue80 != null) {

						if (previousValue80.getOutgoing().size() == o_cast.getOutgoing().size()) {

							java.util.Iterator<activitydiagram.ActivityEdge> it = o_cast.getOutgoing().iterator();
							for (activitydiagramTrace.States.activitydiagram.TracedActivityEdge aPreviousValue : previousValue80
									.getOutgoing()) {
								activitydiagram.ActivityEdge aCurrentValue = it.next();
								if (aPreviousValue != exeToTraced.get(aCurrentValue)) {
									noChange80 = false;
					...
					if (noChange80) {
						newState.getActivityNode_outgoing_Values().add(previousValue80);
				...
				if (o instanceof activitydiagram.JoinNode) {
					...
					// Then we compare the value of the field with the last stored value
					// If same value, we create no local state and we refer to the previous
					List<activitydiagramTrace.States.ActivityNode_heldTokens_Value> localTrace81 = tracedObject
							.getHeldTokensSequence();
					activitydiagramTrace.States.ActivityNode_heldTokens_Value previousValue81 = null;
					...
					if (previousValue81 != null) {
						...
						if (previousValue81.getHeldTokens().size() == o_cast.getHeldTokens().size()) {

							java.util.Iterator<activitydiagram.Token> it = o_cast.getHeldTokens().iterator();
							for (activitydiagramTrace.States.activitydiagram.TracedToken aPreviousValue : previousValue81
									.getHeldTokens()) {
								activitydiagram.Token aCurrentValue = it.next();
								if (aPreviousValue != exeToTraced.get(aCurrentValue)) {
									noChange81 = false;
					...
					if (noChange81) {
						newState.getActivityNode_heldTokens_Values().add(previousValue81);
					...
					// Then we compare the value of the field with the last stored value
					// If same value, we create no local state and we refer to the previous
					List<activitydiagramTrace.States.ActivityNode_incoming_Value> localTrace82 = tracedObject
							.getIncomingSequence();
					activitydiagramTrace.States.ActivityNode_incoming_Value previousValue82 = null;
					...
					if (previousValue82 != null) {
						...
						if (previousValue82.getIncoming().size() == o_cast.getIncoming().size()) {

							java.util.Iterator<activitydiagram.ActivityEdge> it = o_cast.getIncoming().iterator();
							for (activitydiagramTrace.States.activitydiagram.TracedActivityEdge aPreviousValue : previousValue82
									.getIncoming()) {
								activitydiagram.ActivityEdge aCurrentValue = it.next();
								if (aPreviousValue != exeToTraced.get(aCurrentValue)) {
									noChange82 = false;
					...
					if (noChange82) {
						newState.getActivityNode_incoming_Values().add(previousValue82);
					...
					// Then we compare the value of the field with the last stored value
					// If same value, we create no local state and we refer to the previous
					List<activitydiagramTrace.States.ActivityNode_running_Value> localTrace83 = tracedObject
							.getRunningSequence();
					activitydiagramTrace.States.ActivityNode_running_Value previousValue83 = null;
					if (!localTrace83.isEmpty())
						previousValue83 = localTrace83.get(localTrace83.size() - 1);

					boolean content54 = o_cast.isRunning();

					boolean noChange83 = previousValue83 != null && previousValue83.isRunning() == content54;

					if (noChange83) {
						newState.getActivityNode_running_Values().add(previousValue83);
					...
					// Then we compare the value of the field with the last stored value
					// If same value, we create no local state and we refer to the previous
					List<activitydiagramTrace.States.ActivityNode_activity_Value> localTrace84 = tracedObject
							.getActivitySequence();
					activitydiagramTrace.States.ActivityNode_activity_Value previousValue84 = null;
					if (!localTrace84.isEmpty())
						previousValue84 = localTrace84.get(localTrace84.size() - 1);

					storeAsTracedObject(o_cast.getActivity());

					activitydiagramTrace.States.activitydiagram.TracedActivity content55 = null;
					if (o_cast.getActivity() != null)
						content55 = ((activitydiagramTrace.States.activitydiagram.TracedActivity) exeToTraced
								.get(o_cast.getActivity()));

					boolean noChange84 = previousValue84 != null && previousValue84.getActivity() == content55;

					if (noChange84) {
						newState.getActivityNode_activity_Values().add(previousValue84);
						...
						newValue.setActivity(content55);
					...
					// Then we compare the value of the field with the last stored value
					// If same value, we create no local state and we refer to the previous
					List<activitydiagramTrace.States.NamedElement_name_Value> localTrace85 = tracedObject
							.getNameSequence();
					activitydiagramTrace.States.NamedElement_name_Value previousValue85 = null;
					if (!localTrace85.isEmpty())
						previousValue85 = localTrace85.get(localTrace85.size() - 1);

					java.lang.String content56 = o_cast.getName();

					boolean noChange85 = previousValue85 != null && previousValue85.getName() == content56;

					if (noChange85) {
						newState.getNamedElement_name_Values().add(previousValue85);
					...
					// Then we compare the value of the field with the last stored value
					// If same value, we create no local state and we refer to the previous
					List<activitydiagramTrace.States.ActivityNode_outgoing_Value> localTrace86 = tracedObject
							.getOutgoingSequence();
					activitydiagramTrace.States.ActivityNode_outgoing_Value previousValue86 = null;
					...
					boolean noChange86 = true;
					if (previousValue86 != null) {

						if (previousValue86.getOutgoing().size() == o_cast.getOutgoing().size()) {

							java.util.Iterator<activitydiagram.ActivityEdge> it = o_cast.getOutgoing().iterator();
							for (activitydiagramTrace.States.activitydiagram.TracedActivityEdge aPreviousValue : previousValue86
									.getOutgoing()) {
								activitydiagram.ActivityEdge aCurrentValue = it.next();
								if (aPreviousValue != exeToTraced.get(aCurrentValue)) {
									noChange86 = false;
					...
					if (noChange86) {
						newState.getActivityNode_outgoing_Values().add(previousValue86);
	...
	private void addImplicitStep(activitydiagramTrace.Steps.Step currentStep,
		...
		if (currentStep instanceof activitydiagramTrace.Steps.Activitydiagram_Activity_WriteTrace) {
	...
	public boolean addStep(org.gemoc.execution.engine.mse.engine_mse.MSEOccurrence mseOccurrence) {
		...
		if (mseOccurrence != null && mseOccurrence instanceof activitydiagramTrace.Steps.Step) {
			...
			if (step instanceof activitydiagramTrace.Steps.Activitydiagram_ActivityNode_SendOffers1) {
	...
	public String getDescriptionOfValue(EObject eObject) {
		if (eObject instanceof activitydiagramTrace.States.Variable_initialValue_Value) {
