package org.modelexecution.operationalsemantics.ad.trace.tracemanager;
...
public class ActivitydiagramTraceManager implements ITraceManager {
	...
	private boolean addState(boolean onlyIfChange) {
		...
		for (TreeIterator<EObject> i = executedModel.getAllContents(); i
			...
			if (o instanceof activitydiagram.Trace) {
				...
				List<activitydiagramTrace.Values.Trace_executedNodes_Value> localTrace1 = tracedObject
						.getExecutedNodesTrace();
				activitydiagramTrace.Values.Trace_executedNodes_Value previousValue1 = null;
				...
				if (previousValue1 != null) {
					...
					if (previousValue1.getExecutedNodes().size() == o_cast
							.getExecutedNodes().size()) {

						java.util.Iterator<activitydiagram.ActivityNode> it = o_cast
								.getExecutedNodes().iterator();
						for (activitydiagramTrace.Traced.activitydiagram.TracedActivityNode aPreviousValue : previousValue1
								.getExecutedNodes()) {
				...
				if (noChange1) {
					newState.getTrace_executedNodes_Values()
							.add(previousValue1);
			...
			if (o instanceof activitydiagram.Activity) {
				...
				// Then we compare the value of the field with the last stored value
				// If same value, we create no local state and we refer to the previous
				List<activitydiagramTrace.Values.Activity_trace_Value> localTrace2 = tracedObject
						.getTraceTrace();
				activitydiagramTrace.Values.Activity_trace_Value previousValue2 = null;
				...
				boolean noChange2 = previousValue2 != null
						&& previousValue2.getTrace() == content1;

				if (noChange2) {
					newState.getActivity_trace_Values().add(previousValue2);
			...
			if (o instanceof activitydiagram.BooleanValue) {
				...
				// Then we compare the value of the field with the last stored value
				// If same value, we create no local state and we refer to the previous
				List<activitydiagramTrace.Values.BooleanValue_value_Value> localTrace3 = tracedObject
						.getValueTrace();
				activitydiagramTrace.Values.BooleanValue_value_Value previousValue3 = null;
				...
				boolean noChange3 = previousValue3 != null
						&& previousValue3.isValue() == content2;

				if (noChange3) {
					newState.getBooleanValue_value_Values().add(previousValue3);
			...
			if (o instanceof activitydiagram.InputValue) {
				...
				// Then we compare the value of the field with the last stored value
				// If same value, we create no local state and we refer to the previous
				List<activitydiagramTrace.Values.InputValue_value_Value> localTrace7 = tracedObject
						.getValueTrace();
				activitydiagramTrace.Values.InputValue_value_Value previousValue7 = null;
				if (!localTrace7.isEmpty())
					previousValue7 = localTrace7.get(localTrace7.size() - 1);

				storeAsTracedObject(o_cast.getValue());

				activitydiagramTrace.Traced.activitydiagram.TracedValue content3 = null;
				if (o_cast.getValue() != null)
					content3 = ((activitydiagramTrace.Traced.activitydiagram.TracedValue) exeToTraced
							.get(o_cast.getValue()));

				boolean noChange7 = previousValue7 != null
						&& previousValue7.getValue() == content3;

				if (noChange7) {
					newState.getInputValue_value_Values().add(previousValue7);
				...
				// Then we compare the value of the field with the last stored value
				// If same value, we create no local state and we refer to the previous
				List<activitydiagramTrace.Values.InputValue_variable_Value> localTrace8 = tracedObject
						.getVariableTrace();
				activitydiagramTrace.Values.InputValue_variable_Value previousValue8 = null;
				if (!localTrace8.isEmpty())
					previousValue8 = localTrace8.get(localTrace8.size() - 1);

				storeAsTracedObject(o_cast.getVariable());

				activitydiagramTrace.Traced.activitydiagram.TracedVariable content4 = null;
				if (o_cast.getVariable() != null)
					content4 = ((activitydiagramTrace.Traced.activitydiagram.TracedVariable) exeToTraced
							.get(o_cast.getVariable()));

				boolean noChange8 = previousValue8 != null
						&& previousValue8.getVariable() == content4;
			...
			if (o instanceof activitydiagram.Offer) {
				...
				// Then we compare the value of the field with the last stored value
				// If same value, we create no local state and we refer to the previous
				List<activitydiagramTrace.Values.Offer_offeredTokens_Value> localTrace9 = tracedObject
						.getOfferedTokensTrace();
				activitydiagramTrace.Values.Offer_offeredTokens_Value previousValue9 = null;
				...
				if (previousValue9 != null) {
					...
					if (previousValue9.getOfferedTokens().size() == o_cast
							.getOfferedTokens().size()) {

						java.util.Iterator<activitydiagram.Token> it = o_cast
								.getOfferedTokens().iterator();
						for (activitydiagramTrace.Traced.activitydiagram.TracedToken aPreviousValue : previousValue9
								.getOfferedTokens()) {
							activitydiagram.Token aCurrentValue = it.next();
							if (aPreviousValue != exeToTraced
									.get(aCurrentValue)) {
			...
			if (o instanceof activitydiagram.IntegerValue) {
				...
				// Then we compare the value of the field with the last stored value
				// If same value, we create no local state and we refer to the previous
				List<activitydiagramTrace.Values.IntegerValue_value_Value> localTrace12 = tracedObject
						.getValueTrace();
				activitydiagramTrace.Values.IntegerValue_value_Value previousValue12 = null;
				...
				boolean noChange12 = previousValue12 != null
						&& previousValue12.getValue() == content5;
			...
			if (o instanceof activitydiagram.MergeNode) {
				...
				// Then we compare the value of the field with the last stored value
				// If same value, we create no local state and we refer to the previous
				List<activitydiagramTrace.Values.ActivityNode_heldTokens_Value> localTrace13 = tracedObject
						.getHeldTokensTrace();
				activitydiagramTrace.Values.ActivityNode_heldTokens_Value previousValue13 = null;
				...
				if (previousValue13 != null) {
					...
					if (previousValue13.getHeldTokens().size() == o_cast
							.getHeldTokens().size()) {

						java.util.Iterator<activitydiagram.Token> it = o_cast
								.getHeldTokens().iterator();
						for (activitydiagramTrace.Traced.activitydiagram.TracedToken aPreviousValue : previousValue13
								.getHeldTokens()) {
							activitydiagram.Token aCurrentValue = it.next();
							if (aPreviousValue != exeToTraced
									.get(aCurrentValue)) {
			...
			if (o instanceof activitydiagram.BooleanVariable) {
				...
				boolean noChange15 = previousValue15 != null
						&& previousValue15.getCurrentValue() == content6;
			...
			if (o instanceof activitydiagram.ForkedToken) {
				...
				// Then we compare the value of the field with the last stored value
				// If same value, we create no local state and we refer to the previous
				List<activitydiagramTrace.Values.Token_holder_Value> localTrace16 = tracedObject
						.getHolderTrace();
				activitydiagramTrace.Values.Token_holder_Value previousValue16 = null;
				if (!localTrace16.isEmpty())
					previousValue16 = localTrace16.get(localTrace16.size() - 1);

				storeAsTracedObject(o_cast.getHolder());

				activitydiagramTrace.Traced.activitydiagram.TracedActivityNode content7 = null;
				if (o_cast.getHolder() != null)
					content7 = ((activitydiagramTrace.Traced.activitydiagram.TracedActivityNode) exeToTraced
							.get(o_cast.getHolder()));

				boolean noChange16 = previousValue16 != null
						&& previousValue16.getHolder() == content7;

				if (noChange16) {
					newState.getToken_holder_Values().add(previousValue16);
				...
				// Then we compare the value of the field with the last stored value
				// If same value, we create no local state and we refer to the previous
				List<activitydiagramTrace.Values.ForkedToken_baseToken_Value> localTrace17 = tracedObject
						.getBaseTokenTrace();
				activitydiagramTrace.Values.ForkedToken_baseToken_Value previousValue17 = null;
				if (!localTrace17.isEmpty())
					previousValue17 = localTrace17.get(localTrace17.size() - 1);

				storeAsTracedObject(o_cast.getBaseToken());

				activitydiagramTrace.Traced.activitydiagram.TracedToken content8 = null;
				if (o_cast.getBaseToken() != null)
					content8 = ((activitydiagramTrace.Traced.activitydiagram.TracedToken) exeToTraced
							.get(o_cast.getBaseToken()));

				boolean noChange17 = previousValue17 != null
						&& previousValue17.getBaseToken() == content8;
				...
				if (noChange17) {
					...
					newValue.setBaseToken(content8);
				...
				// Then we compare the value of the field with the last stored value
				// If same value, we create no local state and we refer to the previous
				List<activitydiagramTrace.Values.ForkedToken_remainingOffersCount_Value> localTrace18 = tracedObject
						.getRemainingOffersCountTrace();
				activitydiagramTrace.Values.ForkedToken_remainingOffersCount_Value previousValue18 = null;
				...
				boolean noChange18 = previousValue18 != null
						&& previousValue18.getRemainingOffersCount() == content9;
			...
			if (o instanceof activitydiagram.Input) {
				...
				// Then we compare the value of the field with the last stored value
				// If same value, we create no local state and we refer to the previous
				List<activitydiagramTrace.Values.Input_inputValues_Value> localTrace19 = tracedObject
						.getInputValuesTrace();
				activitydiagramTrace.Values.Input_inputValues_Value previousValue19 = null;
				...
				if (previousValue19 != null) {
					...
					if (previousValue19.getInputValues().size() == o_cast
							.getInputValues().size()) {

						java.util.Iterator<activitydiagram.InputValue> it = o_cast
								.getInputValues().iterator();
						for (activitydiagramTrace.Traced.activitydiagram.TracedInputValue aPreviousValue : previousValue19
								.getInputValues()) {
				...
				if (noChange19) {
					newState.getInput_inputValues_Values().add(previousValue19);
			...
			if (o instanceof activitydiagram.DecisionNode) {
				...
				// Then we compare the value of the field with the last stored value
				// If same value, we create no local state and we refer to the previous
				List<activitydiagramTrace.Values.ActivityNode_heldTokens_Value> localTrace22 = tracedObject
						.getHeldTokensTrace();
				activitydiagramTrace.Values.ActivityNode_heldTokens_Value previousValue22 = null;
				...
				if (previousValue22 != null) {
					...
					if (previousValue22.getHeldTokens().size() == o_cast
							.getHeldTokens().size()) {
	...
	public String getDescriptionOfValue(EObject eObject) {
		if (eObject instanceof activitydiagramTrace.Values.ActivityEdge_offers_Value) {
